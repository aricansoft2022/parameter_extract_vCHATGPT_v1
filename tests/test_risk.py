    ...
    return [
        {
            ...
            "strategy": _strategy(30.0),
        },
        {
            "priority": 2,
            "original_priority": 3,
            "family_id": "F0002",
            "candidate_fingerprint_sha256": "2" * 64,
            "strategy": _strategy(31.0),
        },
    ]
        ...
        "selected_portfolio_windows": [
            {
                ...
                "name": "d1",
                "trades": [
                    _trade("F0001", "1" * 64, -1.0, 30.0),
                    _trade("F0002", "2" * 64, -2.0, 45.0),
                ],
            },
            {
                "phase": "validation",
                "name": "v1",
                "trades": [
                    _trade("F0001", "1" * 64, -3.0, 60.0),
                    _trade("F0002", "2" * 64, -1.5, 40.0),
                ],
            },
        ],
    }
...
def _holdout(selection_sha: str, *, status: str = "PASS") -> dict:
    return {
        ...
        "windows": [
            {
                ...
                "name": "h1",
                "trades": [
                    _trade("F0001", "1" * 64, -2.5, 50.0),
                    _trade("F0002", "2" * 64, -2.0, 35.0),
                ],
            }
        ],
    }
...
def _write_inputs(tmp_path: Path, *, allocation_pct: float = 50.0, holdout_status: str = "PASS"):
    ...
    holdout_path.write_text(
        json.dumps(_holdout(sha256_file(selection_path), status=holdout_status)),
        encoding="utf-8",
    )
...
def test_risk_blocks_instead_of_dropping_family_with_thin_evidence(tmp_path: Path, monkeypatch):
    ...
    holdout = _holdout(sha256_file(selection_path))
    holdout["windows"][0]["trades"] = [
        _trade("F0001", "1" * 64, -2.0, 40.0),
        _trade("F0002", "2" * 64, -2.0, 40.0),
    ]
    holdout_path = tmp_path / "holdout-result.json"
    ...
    assert any(reason.startswith("INSUFFICIENT_FAMILY_RISK_TRADES:F0002") for reason in result["failure_reasons"])
    assert result["selected_set_changed"] is False


def test_risk_result_detects_evidence_mutation(tmp_path: Path, monkeypatch):
    selection_path, holdout_path, risk_path = _write_inputs(tmp_path)
    monkeypatch.setattr(risk_module, "verify_selection_result", lambda payload: [])
    monkeypatch.setattr(risk_module, "verify_holdout_result", lambda payload: [])
    result = run_risk(selection_path, holdout_path, risk_path)
    ...
    assert any("summary does not match" in problem for problem in problems)
